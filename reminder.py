...
# all fuctions and processes for interaction with users
        ...
        for chat_id, text, time_not, reminder_date, priority, reminder_id, before_time in result:
            ...
            if time_not == real_time:
                ...
                if priority == 1 and before_time == 0:
                    ...
                    btn1 = tb.types.InlineKeyboardButton('Delay reminder', callback_data=f'delay{reminder_id}')
                    btn2 = tb.types.InlineKeyboardButton("Solve an exercise", callback_data=f'solve{reminder_id}')
...
if __name__ == "__main__":
    interactions = Process(target=user_interactions)
    notifications = Process(target=handle_notifications)
